...
module.exports = class FelaquaDevice extends Homey.Device {
  ...
  async update(deviceData) {
    ...
    if (this.getCapabilityValue(ALARM_OFFLINE) === null) {
      await this.setCapabilityValue(ALARM_OFFLINE, offlineState);
    }
    this.tryTriggerOfflineChange(offlineState);
  }
  ...
  /**
   * try triggering offline change
   *
   * @param newOfflineState
   */
  tryTriggerOfflineChange(newOfflineState) {
    const currentOfflineState = this.getCapabilityValue(ALARM_OFFLINE);
    this.homey.app.logMessage('log', `change offline state of ${this.getName()} - new ${newOfflineState} - current ${currentOfflineState}`);
    if (currentOfflineState !== newOfflineState) {
      this.setDeviceCapabilityValue(ALARM_OFFLINE, newOfflineState);
      if (newOfflineState) {
