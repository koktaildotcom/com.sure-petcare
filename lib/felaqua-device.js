...
// minimum rise in fill percentage to count as a refill instead of measurement noise
const REFILL_MINIMUM_INCREASE = 10;
...
module.exports = class FelaquaDevice extends Homey.Device {
  ...
  async update(deviceData) {
    ...
    if (this.homey.app.hasProperties(deviceData, ['status', 'bowl_status', 0, 'fill_percent'])) {
  ...
  tryTriggerChange(newValue) {
    ...
    if (currentValue !== newValue) {
      ...
      if (newValue - currentValue >= REFILL_MINIMUM_INCREASE) {
      ...
      this.homey.flow.getDeviceTriggerCard('felaqua_fill_level_less_than')
        .trigger(this, fillLevelData, {
          fill_level: newValue,
          previous_fill_level: currentValue,
        })
        .catch(this.error);
    }
  }
  ...
  async checkPetChange(storedPet, pet) {
    ...
    if (!this.homey.app.hasProperties(storedPet, ['status', 'drinking', 'at'])) {
      // store the first in cache, keeping other device statuses like feeding intact
      storedPet.status = {
        ...storedPet.status,
        drinking: {
          at: null,
    ...
    if (storedPet.status.drinking.at !== pet.status.drinking.at) {
      ...
      if (deviceId === this.getDeviceId() || deviceId === '') {
        ...
        if (!Array.isArray(drinking.change) || drinking.change.length === 0) {
          return;
        }
