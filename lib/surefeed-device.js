...
module.exports = class SurefeedDevice extends Homey.Device {
  ...
  tryTriggerChange(bowlType, newValue) {
    ...
    const currentRoundedValue = Math.round((currentValue + Number.EPSILON) * 100) / 100;
    let rounded = Math.round((newValue + Number.EPSILON) * 100) / 100;
    rounded = (rounded <= 0) ? 0 : rounded;
    if (currentRoundedValue !== rounded) {
      if (currentRoundedValue < rounded) {
        this.homey.flow.getTriggerCard('bowl_has_been_refilled').trigger(this, {
          bowl: this.homey.__(bowlType),
          amount: (Math.round(((rounded - currentValue) + Number.EPSILON) * 100) / 100),
        });
      }
