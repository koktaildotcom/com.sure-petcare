...
module.exports = class SurefeedDevice extends Homey.Device {
  ...
  tryTriggerChange(bowlType, newValue) {
    ...
    if (value !== rounded) {
      ...
      this.homey.flow.getCard('trigger', 'weight_changed').trigger(this, {
      ...
      if (deviceId === this.getId() || deviceId === '') {
        ...
        if (await this.getSetting('bowlType') === SINGLE) {
          ...
          if (rounded > 0) {
            ...
            this.homey.flow.getCard('trigger', 'pet_has_eating').trigger(this, petData);
            this.homey.flow.getCard('trigger', 'specific_pet_has_eating').trigger(this, petData, {
              petId: pet.id,
            });
          }
        } else if (await this.getSetting('bowlType') === DOUBLE) {
