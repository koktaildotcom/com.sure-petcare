    ...
    if (storedPet.position.since !== pet.position.since) {
      ...
      if (deviceId === this.getId() || deviceId === '') {
        ...
        if (pet.position.where === 1) {
          this.homey.flow.getTriggerCard('pet_home').trigger(this, petData);
          this.homey.flow.getTriggerCard('specific_pet_home').trigger(this, petData, {
            petId: pet.id,
          });
        }
        if (pet.position.where === 2) {
          this.homey.flow.getTriggerCard('pet_away').trigger(this, petData);
          this.homey.flow.getTriggerCard('specific_pet_away').trigger(this, petData, {
            petId: pet.id,
          });
        }
      }
