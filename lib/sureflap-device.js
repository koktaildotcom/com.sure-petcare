...
module.exports = class SureflapDevice extends Homey.Device {
  ...
  async update(deviceData) {
    if (this.hasCapability('measure_battery')) {
      const batteryStatus = this.app.getProperty(
        deviceData,
        ['status', 'battery'],
      );
      ...
      await this.setCapabilityValue('measure_battery', await this.app.calculateBatteryPercentage(batteryStatus));
    }

    const lockStateIndex = this.app.getProperty(
      deviceData,
      ['status', 'locking', 'mode'],
    );
  ...
  async registerListeners() {
    this.registerCapabilityListener('lock_mode', async state => {
      const lockStateIndex = this.app.getProperty(lockStates, [state]);
  ...
  async checkPetChange(storedPet, pet) {
    if (this.hasProperty(pet, 'position') && storedPet.position && storedPet.position.since !== pet.position.since) {
      ...
      const position = this.app.getProperty(pet, ['position']);
      let deviceId = '';
