...
module.exports = class SureflapDevice extends Homey.Device {
  ...
  async onInit() {
    ...
    SurePetcare.registerDevice(this);
  }
  ...
  async registerFlowCards() {
    this.lockModeIs = this.homey.flow.getConditionCardCard('lock_mode_is');
  ...
  async checkPetChange(storedPet, pet) {
    if (!Homey.app.hasProperties(pet, ['position', 'since'])) {
      return;
    }

    if (!Homey.app.hasProperties(storedPet, ['position', 'since'])) {
    ...
    if (storedPet.position.since !== pet.position.since) {
      Homey.app.logMessage('log', `change position for ${pet.name}`);
      ...
      if (deviceId === this.getId() || deviceId === '') {
        ...
        if (pet.position.where === 1) {
          this.homey.flow.getCard('trigger', 'pet_home').trigger(this, petData);
          this.homey.flow.getCard('trigger', 'specific_pet_home').trigger(this, petData, {
            petId: pet.id,
          });
        }
        if (pet.position.where === 2) {
          this.homey.flow.getCard('trigger', 'pet_away').trigger(this, petData);
          this.homey.flow.getCard('trigger', 'specific_pet_away').trigger(this, petData, {
            petId: pet.id,
          });
        }
      }
